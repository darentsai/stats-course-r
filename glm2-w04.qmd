...
aggregate(birthweight ~ smoking, data = smk_wt, FUN = mean)
aggregate(birthweight ~ smoking, data = smk_wt, FUN = sd)
...
令容許一次成對比較犯錯的機率為$\alpha^*$，容許整體犯錯的機率為$\alpha$，則
...
為了避免型 I 誤差膨脹，可將容許一次成對比較犯錯的機率設為 $\frac{\alpha}{3}$
(等同於將進行一次成對比較的 *p*-value 乘以3)，
