...
- Normality
   - Check using **Shapiro-Wilk test**: `shapiro.test()`
   - If not met, use non-parametric **Kruskal-Wallis test**: `kruskal.test()`
- Equality of variances
   - Check using **Bartlett's test** or **Levene's test**: `bartlett.test()` or `car::leveneTest()`
   - If not met, use **Welch's *F*-test**: `oneway.test(..., var.equal = FALSE)`
...
#### Tukey's HSD (honestly significant difference) test {.unnumbered}
